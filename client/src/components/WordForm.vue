  ...
  <div class="relative h-screen overflow-x-auto shadow-md sm:rounded-lg flex justify-center items-start mt-10">
    <div class="w-2/3 text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      <form @submit.prevent="onSubmit" class="grid grid-cols-2">
        ...
        <input class="px-2 py-4 border-2 border-l-0 border-gray-600 border-b-0 text-center w-1/3 text-lg text-gray-900  focus:outline-none" v-model="form.english">
        ...
        <input class="px-2 py-4 border-2 border-l-0 border-gray-600 text-center w-1/3 text-lg text-gray-900 focus:outline-none" v-model="form.german">
        <button class="px-2 py-4 text-lg font-medium text-green-400 hover:text-green-600">Submit</button>
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  methods : {
    onSubmit() {
      if (this.form.german === '' || this.form.english === '') {
